            ...
            if self.inWatchOpenOrder:
                try:
                    ...
                    if relevant_orders:
                        ...
                        logger.info(f"收到的相关订单ID: {[str(o['id']) for o in relevant_orders]}")
                        ...
                        for order in relevant_orders:
                            ...
                            if str(order['id']) == str(order_id):
                                ...
                                if order['status'] in ['closed', 'filled']:
                                    ...
                                    logger.info(f"{self.symbolName}订单{order_id}已成交: {order['side']} {order['amount']} @ {order['price']}")
                                break
                        
                        # 如果订单在相关订单中找不到，说明可能已经完全成交并从未成交列表中移除
                        if not order_found:
                            logger.info(f"{self.symbolName}订单{order_id}已从未成交列表中消失，可能已完全成交")
                    ...
                    if filled_orders or len([oid for oid in self.openOrders if not any(str(o['id']) == str(oid) for o in relevant_orders)]) > 0:
                        logger.info(f"{self.symbolName}检测到订单成交，通知TradeManager进行后续处理")
                        # 先停止当前监听，避免重复处理
                        self.inWatchOpenOrder = False
                        self.openOrders = []
