...
所有波动率相关参数已统一移动到 `config/config.py` 文件中的 `VolatilityConfig` 类。
...
```python
# 波动率倍数参数
MIN_SPREAD_MULTIPLIER = 2           # minSpread倍数
BASE_SPREAD_MULTIPLIER = 4          # baseSpread倍数
MAX_SPREAD_MULTIPLIER = 16          # maxSpread倍数
...
系统默认监听1分钟K线数据，如需调整可在配置文件中修改 `KLINE_TIMEFRAME` 参数。

支持的时间框架：`'1m'`, `'3m'`, `'5m'`, `'15m'`, `'30m'`, `'1h'`, `'2h'`, `'4h'`, `'6h'`, `'8h'`, `'12h'`, `'1d'`

### 3.5 配置修改方法

要修改波动率相关配置，请编辑 `config/config.py` 文件中的 `VolatilityConfig` 类：

```python
class VolatilityConfig:
    # 修改ATR计算周期
    ATR_PERIOD = 14  # 从默认的10改为14
    
    # 修改价差倍数
    MIN_SPREAD_MULTIPLIER = 1.5   # 降低最小价差倍数
    BASE_SPREAD_MULTIPLIER = 3    # 降低基础价差倍数
    MAX_SPREAD_MULTIPLIER = 12    # 降低最大价差倍数
    
    # 启用波动率平滑
    VOLATILITY_SMOOTHING = True
    SMOOTHING_FACTOR = 0.2        # 增加平滑程度
```
...
- **v1.1** (2025-08-05): **重要更新** - 将所有配置参数移动到 `config/config.py` 统一管理，新增波动率平滑功能，增加更多可配置参数，优化配置管理和系统可维护性
